...
| Problem | Solution |
| :------------ | :----------: |
| Generating a triangular pattern, taking the number of rows as the input, giving triangular pattern of fibonacci elements. | [fibonacciTraingle.cpp](Patterns_With_C++/fibonacciTraingle.cpp) |
| Generating a double - sided arrow pattern, out of integers, taking the number of rows (say n, where n > 3) as the input. | [PatternDoubleSidedArrow.cpp](Patterns_With_C++/PatternDoubleSidedArrow.cpp) |
| Generate a hollow rhombus of asterisks, taking the numbers of asterisks ( * ), forming a side as the input. | [PatternHollowRhombus.cpp](Patterns_With_C++/PatternHollowRhombus.cpp)|
| Generate a pattern out of asterisks, with a diamond - shaped hollow in the middle of a square. | [PatternHollowDiamond.cpp](Patterns_With_C++/PatternHollowDiamond.cpp)
| Generate an inverted hour glass pattern | [PatternInvertedHourGlass.cpp](Patterns_With_C++/PatternInvertedHourGlass.cpp)|
| Generate a triangular pattern with each row having a combination of increasing and decreasing patterns respectively. |[increasingDecreasingTriangle.cpp](Patterns_With_C++/increasingDecreasingTriangle.cpp)|
| Generate a triangular pattern with each row having integers (row_number), with zeroes in the middle, and the number of zeroes is equal to (row_number - 2), followed by the same integer again. |[ZeroesBetweenInts.cpp](Patterns_With_C++/ZeroesBetweenInts.cpp)|
